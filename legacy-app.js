...
function hijriText(){var h=settings.hijri;if(!h)return'—';if(typeof h==='string')return h;if(h.text)return h.text;if(h.date)return h.date;if(h.label)return h.label;if(h.start&&h.month&&h.year){var a=h.start.split('-');if(a.length===3){var start=Date.UTC(parseInt(a[0],10),parseInt(a[1],10)-1,parseInt(a[2],10));var p=parts(),today=Date.UTC(p.year,p.month-1,p.day);var day=Math.floor((today-start)/86400000)+1;if(day>0&&day<=30)return day+' '+h.month+' '+h.year+' هـ';}}var hp=[];if(h.day)hp.push(h.day);if(h.month)hp.push(h.month);if(h.year)hp.push(h.year);return hp.length?hp.join(' '):'—';}
...
function loadCalendar(){var urls=['./horaires_priere_zone_sud_reunion_complet.json?v=20260831offline2','./horaires_priere_zone_sud_reunion_complet.json'];var i=0;function attempt(){if(i>=urls.length){if(loadLocalCalendar()){render();return;}showError();return;}var u=urls[i++];xhr(u,{},function(status,text){if(status>=200&&status<300&&acceptCalendar(text))return;attempt();});}attempt();}
