...
function acceptCalendar(text){try{var data=JSON.parse(text);if(data&&data.horaires&&data.horaires.length){calendar=data.horaires;render();return true;}}catch(e){}return false;}
function loadCalendar(){var urls=['./horaires_priere_zone_sud_reunion_complet.json?v=20260831fix2','./horaires_priere_zone_sud_reunion_complet.json'];var i=0;function attempt(){if(i>=urls.length){showError();return;}var u=urls[i++];xhr(u,{},function(status,text){if(status>=200&&status<300&&acceptCalendar(text))return;attempt();});}attempt();}
...
document.title=(config.mosqueName||'Mosquée Anas Ibn Malik')+' — Horaires de prière';tick();setInterval(tick,1000);loadSettings(loadCalendar);setInterval(function(){render();},30000);setInterval(function(){loadSettings(function(){render();});},60000);
})();