...
function adhan(name,fallback){if(name==='Maghrib')return fallback;if(settings.adhan&&settings.adhan[name])return settings.adhan[name];if(config.adhan&&config.adhan[name])return config.adhan[name];return fallback;}
function hijriSource(){return settings.hijri&&settings.hijri.month?settings.hijri:(config.hijri||{});}
function hijriText(){var h=hijriSource();if(!h)return'—';if(typeof h==='string')return h;if(h.text)return h.text;if(h.date)return h.date;if(h.label)return h.label;if(h.start&&h.month&&h.year){var a=h.start.split('-');if(a.length===3){var start=Date.UTC(parseInt(a[0],10),parseInt(a[1],10)-1,parseInt(a[2],10));var p=parts(),today=Date.UTC(p.year,p.month-1,p.day);var day=Math.floor((today-start)/86400000)+1;if(day>0&&day<=30)return day+' '+h.month+' '+h.year+' هـ';}}var hp=[];if(h.day)hp.push(h.day);if(h.month)hp.push(h.month);if(h.year)hp.push(h.year);return hp.length?hp.join(' '):'—';}
...
function loadCalendar(){var urls=['./horaires_priere_zone_sud_reunion_complet.json?v=20260831offline3','./horaires_priere_zone_sud_reunion_complet.json'];var i=0;function attempt(){if(i>=urls.length){if(loadLocalCalendar()){render();return;}showError();return;}var u=urls[i++];xhr(u,{},function(status,text){if(status>=200&&status<300&&acceptCalendar(text))return;attempt();});}attempt();}
function loadSettings(done){if(!sb.enabled||!sb.url||!sb.anonKey){done();return;}var url=sb.url+'/rest/v1/mosque_settings?id=eq.anas-ibn-malik&select=prayers,adhan,jumua,hijri,announcements,updated_at&_t='+Date.now();xhr(url,{apikey:sb.anonKey,Authorization:'Bearer '+sb.anonKey,Accept:'application/json'},function(status,text){if(status>=200&&status<300){try{var rows=JSON.parse(text);if(rows&&rows.length){settings=rows[0]||{};saveLocalSettings();}}catch(e){}}done();});}
